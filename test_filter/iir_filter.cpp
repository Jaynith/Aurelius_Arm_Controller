...
float iir_filter::filter_data(float data_in){
  ...
  /*
  Serial.print(_filter_buffer[0][FILTER_BUFFER_LENGTH-1]);
  Serial.print(" | ");
  Serial.println(_filter_buffer[1][FILTER_BUFFER_LENGTH-1]);
  */
  ...
  for (x = 0; x < _filter_buffer_width; x++){
    ...
    if (x < _den_size - 1){
      ...
      //Serial.println(_filter_buffer[x][FILTER_BUFFER_LENGTH-1]);
    }
